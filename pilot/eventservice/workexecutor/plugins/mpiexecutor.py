...
class MPIService(threading.Thread):
    ...
    def put(self, req):
        if req.message_type == MPIMessage.MessageType.Request:
            logging.info("put request to be delivered: %s" % req)
            if req.wait_for_response:
                logging.info("Record request to wait for response: %s" % req.message_id)
                self.req_resps[req.message_id] = req
        else:
            logging.info("put response to be delivered: %s" % req)
            if req.message_id in self.req_resps:
                logging.info("To deliver a response for request: %s" % req.message_id)
        ...
        while not self.is_stop():
            ...
            if req:
                if req.message_type == MPIMessage.MessageType.Request:
                    ...
                    logging.info("received response: %s" % req)
                    if req.message_id in self.req_resps:
                        logging.info("Record response to wait for client to fetch: %s" % req.message_id)
                        self.req_resps[req.message_id].response = req
                    else:
                        logging.warn("No corresponding request for response, put it to the receiver queue directly: %s" % req.message_id)
                        self.receiver_queue.put(req)
...
class MPIManager(threading.Thread):
    ...
    def request_jobs(self):
        logging.info("Requesting jobs")
        jobs = self.communication_manager.get_jobs(njobs=1)
        logging.info("Received jobs: %s" % jobs)
        for job in jobs:
            self.jobs[job['PandID']] = job
            if 'ranks' not in job:
                job['ranks'] = 1
            for i in range(job['ranks']):
                self.job_assign_queue.append(job['PandID'])

    def get_jobs(self, req):
        logging.info("Getting jobs for req: %s" % req)
        if not self.job_assign_queue:
            self.request_jobs()

        job = None
        while self.job_assign_queue:
            job_id = self.job_assign_queue.pop(0)
            if req.from_rank not in self.rank_jobs:
                self.rank_jobs[req.from_rank] = []
            if job_id not in self.rank_jobs[req.from_rank]:
                self.rank_jobs[req.from_rank].append(job_id)
                job = self.jobs[job_id]
                break

        resp_attr = req.generate_response()
        if self.current_job['pandaid'] not in self.rank_jobs[self.rank]:
            resp_attr['content'] = job
        else:
