...
from pilot.control.job import get_job_definition_from_server, update_es_dispatcher_data, create_job
...
class ACTCommunicator(BaseCommunicator):
    def __init__(self, *args, **kwargs):
        super(ACTCommunicator, self).__init__(args, kwargs)
    ...
    def get_jobs(self, req):
        ...
        try:
            ...
            data = {'node': req.node,
                    'mem': req.mem,
                    'getProxyKey': 'False',
                    'computingElement': req.queue,
                    'diskSpace': req.disk_space,
                    'siteName': req.site,
                    'prodSourceLabel': req.job_label,
                    'workingGroup': req.working_group,
                    'cpu': req.cpu}
