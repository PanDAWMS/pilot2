...
def verify_setup_command(cmd):
    ...
    logger.debug('atlashpc user verify_setup_command cmd: {}'.format(cmd))

    # if running in a harvester provided container with non asetup setup skip verify step.
    # test if environmental variable HARVESTER_CONTAINER_RELEASE_SETUP_FILE is define
    standalone_container = False
    setupfile = os.environ.get('HARVESTER_CONTAINER_RELEASE_SETUP_FILE', '')
    if setupfile != "":
        standalone_container = True

    if standalone_container:
        logger.info('atlashpc user verify_setup_command - skip verify step')
    else:
...
def get_setup_command(job, prepareasetup):
    ...
    Overload command to handle the case where environmental variables are set -
    ...
    if job.swrelease == 'NULL' or '--containerImage' in job.jobparams:
        msg = 'user - atlashpc get_setup_command return value: {}'.format(str(cmd))
        logger.debug(msg)
        return cmd
    ...
    if standalone_container:
        ...
        if os.environ.get('HARVESTER_LD_LIBRARY_PATH', '') != "":
            #cmd += "export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:{};".format(os.environ.get('HARVESTER_LD_LIBRARY_PATH',''))
        ...
        if os.environ.get('HARVESTER_PYTHONPATH', '') != "":
            #cmd += "export PYTHONPATH=$PYTHONPATH:{};".format(os.environ.get('HARVESTER_PYTHONPATH',''))
