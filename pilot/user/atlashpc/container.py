...
from pilot.user.atlashpc.setup import get_asetup
from pilot.user.atlashpc.setup import get_file_system_root_path
...
def do_use_container(**kwargs):
    ...
    logger.info('do_use_container called')
    ...
    if job:
        ...
        if job.is_analysis() and job.imagename:
            ...
            logger.debug('do_use_container container_name: {}'.format(str(container_name)))
    ...
    logger.debug('do_use_container return value: {}'.format(str(use_container)))
...
def wrapper(executable, **kwargs):
    ...
    logger.debug('wrapper - executable : {}'.format(executable))
    logger.debug('wrapper - workdir : {}'.format(workdir))
    ...
    if config.Container.setup_type == "ALRB" and job and not job.imagename:
        fctn = alrb_wrapper
        logger.debug('wrapper - fctn = alrb_wrapper')
    else:
        fctn = singularity_wrapper
        logger.debug('wrapper - fctn = singularity_wrapper')
