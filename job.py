    ...
    def stage_out(self):
        ...
        if self.pilot.args.harvester is False:            
            ...
            if os.path.exists(readJsonPath):
                ...
                try:
                    ...
                    oJobReport = os.path.join(self.pilot.args.harvester_workdir, "jobReport.json")
                    if not os.path.exists(oJobReport):
                        shutil.copy(readJsonPath, oJobReport)
                except:
                    pass
            else:
        ...
        for f in self.output_files:
            ...
            if self.is_log_file(f):
                athena_logs=['transform.stdout','transform.stderr','%s.log' % self.id,'pilot.log']
                ...
                mem_logs = glob.glob('mem.*')
                if len(mem_logs) > 0:
                    athena_logs+= mem_logs
                other_logs = glob.glob('log.*')
                if len(other_logs) > 0:
                    athena_logs+= other_logs
